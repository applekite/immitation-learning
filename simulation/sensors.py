...
class CameraSensor():
    """
    Semantic Segmentation Camera Sensor for autonomous driving.
    Uses semantic segmentation camera to provide class-labeled pixel information
    instead of RGB colors. This helps the agent understand scene structure better.
    """

    def __init__(self, vehicle):
        # Use semantic segmentation camera (SSC_CAMERA) instead of RGB
        self.sensor_name = SSC_CAMERA  # 'sensor.camera.semantic_segmentation'
        self.parent = vehicle
    ...
    def _set_camera_sensor(self, world):
        """
        Setup semantic segmentation camera sensor.
        Returns semantic segmentation images with class labels for each pixel.
        """
        front_camera_bp = world.get_blueprint_library().find(self.sensor_name)
        front_camera_bp.set_attribute('image_size_x', f'256')
    ...
    def _get_front_camera_data(weak_self, image):
        """
        Process semantic segmentation image data.
        Converts raw image to CityScapes palette format for semantic segmentation.
        Each pixel value represents a semantic class (road, vehicle, building, etc.)
        """
        self = weak_self()
        if not self:
            return
        # Convert to CityScapes palette - required for semantic segmentation
        # This maps each pixel to a semantic class color
        image.convert(carla.ColorConverter.CityScapesPalette)
        placeholder = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
        # Reshape to standard (height, width, channels)
        placeholder1 = placeholder.reshape((image.height, image.width, 4))
        target = placeholder1[:, :, :3]  # Extract RGB channels (semantic class colors)
...
class CollisionSensor:
    ...
    def _on_collision(weak_self, event):
        ...
        Detects collisions with any object including walls, vehicles, and obstacles.
        """
        self = weak_self()
        if not self:
            return
        
        # Get collision event details
        other_actor = event.other_actor
        impulse = event.normal_impulse
        intensity = math.sqrt(impulse.x ** 2 + impulse.y ** 2 + impulse.z ** 2)
        
        # Only record collisions with significant intensity (filter out minor contacts)
        if intensity > 0.1:  # Minimum threshold to avoid false positives
            self.collision_data.append(intensity)
            # Debug: print collision info
            actor_type = other_actor.type_id if other_actor else "unknown"
            print(f"[COLLISION EVENT] Type: {actor_type}, Intensity: {intensity:.2f}")
